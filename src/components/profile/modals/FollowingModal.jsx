      ...
      <div className=" max-h-[60dvh] border- overflow-y-auto no-scrollbar py-4 px-6 ">
        {filteredItems?.map((person) => {
          return (
            <div key={person._id}>
              <div
                className={`flex w-full items-center border-b- justify-between p-2 sm:px-0 md:px-2 hover:bg-gray-100 hover:text-brandprimary`}
              >
                <div className="flex-1">
                  <Link
                    className="flex items-center w-fit"
                    href={`/profile/${person?.user_name || ""}`}
                  >
