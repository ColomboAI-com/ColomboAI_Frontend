...
const FollowersModal = ({followersData}) => {

    const { setIsFollowerModalOpen, handleFollower } = useContext(UserProfileContext);

    const [searchUser, setSearchUser] = useState('');
    const [filteredItems, setFilteredItems] = useState(followersData);

    useEffect(() => {
        setFilteredItems(() => {
            return followersData?.filter((d) => {
                return d.name.toLowerCase().includes(searchUser.toLowerCase());
            });
        });
    }, [searchUser,followersData]);
    ...
    return (
        <div className=" max-h-[80vh]">
            <div className=" flex justify-between px-8 py-6 border-b-2">
                ...
                <button className="outline-none focus:ring-offset-0 focus:ring-0" onClick={() => setIsFollowerModalOpen(false)}>
                ...
                return (
                    <div key={person._id}>
                        <div className={`flex w-full items-center border-b- justify-between p-2 hover:bg-gray-100 hover:text-brandprimary`}>
                            <div className="flex-1">
                                <div className="flex items-center">
                                    <div className="relative flex-shrink-0">
                                        <img src={`${person.profile_picture}`} className="h-12 w-12 rounded-full object-cover" alt="" />
                                    </div>
                            ...
                            <button onClick={() => handleFollower(person)} className={`${person.is_following ? "text-white bg-brandprimary" :"text-brandprimary"} border-[1px] border-brandprimary text-brandprimary rounded-2xl px-6 py-1`}>
