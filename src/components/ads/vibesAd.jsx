...
export default function VibesAd() {
  ...
  useEffect(() => {
    ...
    const initAdDisplayContainer = (videoElement) => {
      ...
      const adDisplayContainer = new window.google.ima.AdDisplayContainer(videoContainerRef.current, videoElement);
      ...
      adsRequest.nonLinearAdSlotHeight = videoContainerRef.current.offsetHeight / 3;
    ...
    const onAdsManagerLoaded = (adsManagerLoadedEvent, videoElement) => {
      ...
      adsManager.addEventListener(window.google.ima.AdEvent.Type.CONTENT_PAUSE_REQUESTED, () => videoElement.pause());
      adsManager.addEventListener(window.google.ima.AdEvent.Type.CONTENT_RESUME_REQUESTED, () => videoElement.play());

      try {
        adsManager.init(videoContainerRef.current.offsetWidth, videoContainerRef.current.offsetHeight, window.google.ima.ViewMode.NORMAL);
        adsManager.start();
      } catch (error) {
