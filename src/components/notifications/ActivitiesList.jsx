        ...
        <div className="text-sm text-navbaraction">
            {mentions.map(mention => (
                <div key={mention.id} className="flex items-center p-2 border rounded space-x-4">
                    <img src={mention.profilePic} alt="Profile" className="w-12 h-12 rounded-full" />
                    <div className="flex-grow">
                        <strong>{mention.user}</strong> {mention.message}
                    </div>
                    <img src={mention.postImage} alt="Post" className="w-12 h-12 rounded" />
                    <button className={"px-4 py-1 text-white rounded bg-brandprimary"}>
                        {mention.followType}
                    </button>
                </div>
            ))}
